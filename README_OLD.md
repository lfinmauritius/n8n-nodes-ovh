...
### Version 0.11.10 - July 2025
- **🚀 VM Backup Operations Enhancement** - Added missing body parameters for VM backup operations:
  - **Enable Backup Enhanced**: Added required `backupDays` parameter (multiOptions: Monday-Sunday) for scheduling VM backups
  - **Restore Backup Improved**: Added optional `restorePointId` parameter to specify exact restore point
  - **Fixed Endpoint Paths**: Corrected VM backup operation endpoints to match OVH API specification
  - **Complete VM Backup Coverage**: All VM backup operations now have proper parameter support
